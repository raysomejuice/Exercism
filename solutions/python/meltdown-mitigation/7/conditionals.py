...
def reactor_efficiency(voltage: float, current: float, theoretical_max_power: float) -> str:
    ...
    power_percentage: float = (generated_power / theoretical_max_power)* 100
...
def fail_safe(temperature: float, neutrons_produced_per_second: float, threshold: float) -> str:
    ...
    """
    threshold_range: float = (temperature * neutrons_produced_per_second) / threshold

    if threshold_range < 0.9:
        return 'LOW'
    elif abs(1 - threshold_range) <= 0.1:
        return 'NORMAL'
    else:
        return 'DANGER'
