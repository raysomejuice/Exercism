...
def generate_seats(number: int) -> Generator[str, None, None]:
    ...
    total_rows = -(-number // seats_per_row)
    
    for row_counter in range(1, total_rows + 1):
        seats_this_row = seats_per_row if (row_counter * seats_per_row <= number) else (number % seats_per_row)
        for seat_counter in range(seats_this_row):
            if row_counter < 13:
                yield str(row_counter) + next(letter)
            else:
                yield str(row_counter + 1) + next(letter)
