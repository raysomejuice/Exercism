...
def approx_average_is_average(hand: list[int]) -> bool:
    ...
    """
    hand.sort()
    avg: float = (min(hand) + max(hand)) / 2
    median: int = hand[len(hand) // 2]
    return avg == card_average(hand) or median == card_average(hand)
...
def average_even_is_average_odd(hand: list[int]) -> bool:
    ...
    """
    hand.sort()
    even_avg: float = sum(hand[::2]) / len(hand[::2])
    odd_avg: float = sum(hand[1::2]) / len(hand[1::2])
