...
def decrement_items(inventory: dict[str, int], items: list[str]) -> dict[str, int]:
    ...
    for entry in items:
        if entry in inventory:
            inventory[entry] = max(inventory[entry] - 1, 0)

#    for entry in items:
#        if entry in inventory and inventory[entry] > 0:
#            inventory[entry] -= 1
...
def list_inventory(inventory: dict[str, int]) -> list[tuple[str, int]]:
    ...
    """
    current_inventory: list[tuple[str, int]] = list(inventory.items())
