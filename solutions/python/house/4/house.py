...
#        rhyme.append(PREFIX + ' '.join(HOUSE[p] for p in range(v, -1, -1)))
    each_verse = ' '.join(HOUSE[p] for p in range(start_verse - 1, -1, -1))
    rhyme = [PREFIX + each_verse]
    for v in range(start_verse, end_verse):
        each_verse = HOUSE[v] + ' ' + each_verse
        rhyme.append(PREFIX + each_verse)    
    return rhyme
